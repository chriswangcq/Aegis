#!/usr/bin/env python3
"""E2E test v4: simplified governance model.

Tests:
  1. Agent lifecycle (register)
  ...
  6. Anti-self-review (same agent blocked)
...
# Same provider is now allowed (cross-provider removed), but same agent is still blocked
p("different-agent same-provider ✓", requests.post(f"{API}/tickets/PR-20/claim", json={"agent_id":"gemini-reviewer"}))
# Release so claude can review
p("release", requests.post(f"{API}/tickets/PR-20/release", json={"agent_id":"gemini-reviewer"}))
...
print("\n═══ 7. Ticket without project → impl submit fails ═══")
